...
class Ledger extends Model
{
    ...
    public function getDebitAmount($accountId,$ledgerId){
        ...
        if($ledger->account_id_from == auth('store')->user()->id && $ledger->manager_id_to == $accountId){
    ...
    public function getCreditAmount($accountId,$ledgerId){
        ...
        $ledger = Ledger::find($ledgerId);
        if($ledger->account_id_to == auth('store')->user()->id && $ledger->manager_id_to == $accountId){
            return $ledger->qty_total;
        }else{
            return false;
        }
    ...
    // }
       ...
       public function countDebit($accountId,$ledgerId){
        ...
        $ledger = Ledger::find($ledgerId);
        // $stockLedgers = Ledger::where('credit_to',$accountId)->get()->pluck('id');
        $stockLedgers = Ledger::where(['manager_id_to'=>$accountId,'account_id_from'=>auth('store')->user()->id])->get()->pluck('id');
        $countCarat = Ledger::whereIn("id",$stockLedgers)->where('created_at','<=',$ledger->created_at)->pluck("qty_total")->all();
        $countCarat = collect($countCarat);
        $countCarat = $countCarat->reduce(function ($carry, $item) {
            return $carry + $item;
    ...
    public function countCredit($accountId,$ledgerId){
        ...
        $ledger = Ledger::find($ledgerId);
        // $stockLedgers = Ledger::where('debit_by',$accountId)->get()->pluck('id');
        $stockLedgers = Ledger::where(['manager_id_from'=>$accountId,'account_id_to'=>auth('store')->user()->id])->get()->pluck('id');
    ...
    // }
     ...
     public function getTotalDebit($accountId){
        ...
        $countCarat = Ledger::where(['manager_id_to'=>$accountId,'account_id_from'=>auth('store')->user()->id])->get()->pluck('qty_total');
    ...
    public function getTotalCredit($accountId){
        ...
        $countCarat = Ledger::where(['manager_id_from'=>$accountId,'account_id_to'=>auth('store')->user()->id])->get()->pluck('qty_total'); 
        $countCarat = collect($countCarat);
        $countCarat = $countCarat->reduce(function ($carry, $item) {
            return $carry + $item;
         }); 
    ...
    public function countProducts($temp_number){
