...
class StoreToStoreBankPaymentLedger extends Model
{
    protected $guarded = [''];
    ...
    public function getDebitAmount($accountId,$ledgerId){
        ...
        $ledger = StoreToStoreBankPaymentLedger::find($ledgerId);
    ...
    public function countDebit($accountId,$ledgerId){
        ...
        $stockLedgers = StoreToStoreBankPaymentLedger::where('voucher_type','6')->whereIn('to',$store1Accounts)->get()->pluck('id');

        $ledger = StoreToStoreBankPaymentLedger::find($ledgerId);
        $countCarat = StoreToStoreBankPaymentLedger::whereIn("id",$stockLedgers)->where('created_at','<=',$ledger->created_at)->pluck("total_amount")->all();
    ...
    public function countCredit($accountId,$ledgerId){
        ...
        $store1Accounts = UserStore::where(['org_id'=>$accountId,'type'=>'user'])->pluck('id')->toArray();
        $store1Accounts = array_merge($store1Accounts,[$accountId]);  
        $stockLedgers = StoreToStoreBankPaymentLedger::where('voucher_type','6')->whereIn('from',$store1Accounts)->get()->pluck('id');
    ...
    public function getTotalDebit($accountId)
    {
        ...
        $countCarat = StoreToStoreBankPaymentLedger::where('voucher_type','6')->whereIn('from',$store1Accounts)->whereIn('to',$store2Accounts)->get()->pluck('total_amount');
        $countCarat = collect($countCarat);
        $countCarat = $countCarat->reduce(function ($carry, $item) {
            return $carry + $item;
         }); 
    ...
    public function getTotalCredit($accountId){
        ...
        $countCarat = StoreToStoreBankPaymentLedger::where('voucher_type','6')->whereIn('to',$store1Accounts)->whereIn('from',$store2Accounts)->get()->pluck('total_amount');
        $countCarat = collect($countCarat);
        $countCarat = $countCarat->reduce(function ($carry, $item) {
            return $carry + $item;
         }); 
