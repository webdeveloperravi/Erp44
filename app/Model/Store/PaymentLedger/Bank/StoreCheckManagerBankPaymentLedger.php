...
class StoreCheckManagerBankPaymentLedger extends Model
{
    protected $guarded = [''];
    ...
    public function getDebitAmount($accountId,$ledgerId){
        ...
        $ledger = StoreCheckManagerBankPaymentLedger::find($ledgerId);
    ...
    public function countDebit($accountId,$ledgerId){
        ...
        $ledger = StoreCheckManagerBankPaymentLedger::find($ledgerId);
        $stockLedgers = StoreCheckManagerBankPaymentLedger::where('voucher_type','6')->where(['to'=>$accountId])->get()->pluck('id');
        $countCarat = StoreCheckManagerBankPaymentLedger::whereIn("id",$stockLedgers)->where('created_at','<=',$ledger->created_at)->pluck("total_amount")->all();
    ...
    public function countCredit($accountId,$ledgerId){
        ...
        $ledger = StoreCheckManagerBankPaymentLedger::find($ledgerId); 
        $stockLedgers = StoreCheckManagerBankPaymentLedger::where('voucher_type','6')->where(['from'=>$accountId])->get()->pluck('id');
    ...
    public function getTotalDebit($accountId){
        ...
        $countCarat = StoreCheckManagerBankPaymentLedger::where('voucher_type','6')->where(['from'=>$accountId])->get()->pluck('total_amount');
        $countCarat = collect($countCarat);
        $countCarat = $countCarat->reduce(function ($carry, $item) {
            return $carry + $item;
         }); 
    ...
    public function getTotalCredit($accountId){
        ...
        $countCarat = StoreCheckManagerBankPaymentLedger::where('voucher_type','6')->where(['to'=>$accountId])->get()->pluck('total_amount');
        $countCarat = collect($countCarat);
        $countCarat = $countCarat->reduce(function ($carry, $item) {
            return $carry + $item;
         }); 
