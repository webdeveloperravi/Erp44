...
class ProductItemsList extends Component
{
    ...
    function applyFilter()
    {
        ...
        if ($this->minRange && $this->maxRange) {
            ...
            $items = $this->getCompleteProductItems($rawItems);
            $filteredItems = $items->whereBetween('productPrice', [$this->minRange, $this->maxRange]);
            $this->proItems = $filteredItems->take(100);
            $this->totalProducts =  $filteredItems->count();
        } else {

            $this->proItems = $this->getCompleteProductItems($rawItems->take(100));
    ...
    function mount(Request $request)
    {
        ...
        if ($request->has('query')) {
            ...
            $rawItems  = Product::where('name', $request['product_name'])->first()->assignCategoryGradeItem(100)->get(); //store specific to be implemented
