...
class StockVerifySecondController extends Controller
{
    public function index(){
        ...
        return view('store.StockVerifySecond.index',compact('products','grades','rattis','stores'));
    }
    ...
    {        
        ...
        if($authUser->type == 'org' || $authUser->type == 'lab')
        {
            ...
            //                                    ->whereHas('ledger', function ($q) use($authUser) {
            //                                        $q->where(['to' =>$authUser->id, 'voucher_type'=>'3']);
            //                                     })->pluck('product_stock_id')->sort()->toArray();
