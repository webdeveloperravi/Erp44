...
class OpeningStockController extends Controller
{
    ...
    public function saveOpeningStock(Request $request)
    {
        ...
        if ($validator->passes())
        {
            ...
            foreach (Session::get('openingStock') as $id => $product)
            {
                ...
                LedgerDetail::create(['ledger_id' => $ledger->id, 'product_stock_id' => $id, 'gin' => $product['gin'], 'product_unit_qty' => $product['exactRatti'], 'product_unit_rate' => $product['exactRate'], 'product_amount' => $product['exactAmount'], 'ledger_status' => 1
    ...
    public function all()
    {
        $products = Session::get('openingStock');
        return view('store.openingStock.all', compact('products'));
