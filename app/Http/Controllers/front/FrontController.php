...
class FrontController extends Controller
{
    ...
    function userPlaceOrder(Request $request)
    {
        ...
        if (session('user_login')) {
            ...
            if ($request['paymentmethod'] == "cash") {
                ...
                $total_amount = $this->getTotalCartAmount($orderMeta['cart_items'], $addOnsAmount, $shippingCharges, $taxAmount, $subTotal) * 100; //total payment amount 
                $order_slug = $newOrder['slug']; //total payment amount
                $redirect_to = 'https://9gem.in/catalogue';
                $encrypt_method = "AES-256-CBC";
                $secret_key = 'AA74CDCC2BBRT935136HH7B63C27'; // user define private key
                $secret_iv = '5fgf5HJ5g27'; // user define secret key
                $key = hash('sha256', $secret_key);
                ...
                return redirect('https://9gem.in/catalogue/api/razorpay?amt=' . $total_amount . '&slug=' . $order_slug . '&user_id=' . $user_id . '&redirect_to=' . $redirect_to); //payment initialization
            }
        }
    }
    ...
    function page_details($url_slug)
    {
        $page = Page::with('user')->where(['url_slug' => $url_slug, 'status' => 1])->first();
        return view('front.page_details', ['page' => $page]);
    }
    ...
    function user_login_post(Request $request)
    {
        $validated_data = Validator::make($request->all(), [
            'phone' => 'required|exists:user_store,phone'
        ]);
        ...
        if (!empty($validated_data->errors()->toArray())) {
            ...
            $phone = $request['phone'];
            $user  = UserStore::where(['phone' => $phone])->first()->toArray();
            $country_code = $user['phone_country_code_id'];
            $res = Http::get('https://api.msg91.com/api/v5/otp?template_id=' . env('MSG91_TEMPLATE_ID_1') . '&mobile=' . $country_code . $phone . '&authkey=' . env('MSG91_AUTHKEY'));

            if ($res->json()['type'] == "success") {
                Session::put('user', $user);
                Session::put('user_id', $user['id']);
                $result['msg_type'] = 'success';
                $result['msg'] = 'Otp sent successfully to ' . $phone;

                return response()->json($result);
            } else {
                $result['msg_type'] = 'error';
                $result['msg'] = 'Please Enter a valid mobile no.';
                return response($result);
            }
        }
    }
    ...
    function verifyOtp(Request $request)
    {
        ...
        if (!empty($validated_data->errors()->toArray())) {
            ...
            $res = Http::get('https://api.msg91.com/api/v5/otp/verify?authkey=' . env('MSG91_AUTHKEY') . '&mobile=' . $country_code . $phone . '&otp=' . $request['otp']);
    ...
    function resendOtp()
    {
        ...
        $res = Http::get('https://api.msg91.com/api/v5/otp/retry?authkey=' . env('MSG91_AUTHKEY') . '&retrytype=text&mobile=' . $country_code . $phone);
    ...
    function getlonglat(Request $request)
    {
        ...
        $res = Http::get('https://www.googleapis.com/oauth2/v2/userinfo?alt=json&access_token=ya29.a0ARrdaM8q0GHvWpmW7GNh328toFPNpZ-tEI8eofUMhSccXKmppjKoURiKzolz2lOOwCrM1AWKdKU7Knq2GBmpHQ3nNDrVkrUBLNoabi5fLRxZdbqY3tk-YVQaebO6z9VsMOF9DzgfAKYxmbdV3cIrtWIbv79r');
    ...
    function aws(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            uploadOnAws($request->file('image'), 'Test', Str::random(10)); //1.file 2.filepath 3.filename
        }
