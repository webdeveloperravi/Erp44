...
use App\Exports\MagentoPriceExport;
...
class FoxproDataImportController extends Controller
{
      ...
      public function updateProduct()
    {
            ...
            TblItem::where('igroupid',53)->update(['erp_product_id'=>16]); 
    }
    ...
    {    
        TblIfvalue::where('origin',null)->chunk(500,function($records){
            foreach($records as $record){
                if($record->item ?? false){

                    if($record->item->erp_product_id == 2){
 
                        $record->update(['origin'=> 1]);
 
                    }elseif($record->item->erp_product_id == 3 || $record->item->erp_product_id == 13){
 
                        $record->update(['origin'=> 8]);
 
                    }elseif($record->item->erp_product_id == 4 ){
 
                        $record->update(['origin'=> 4]);
 
                    }elseif($record->item->erp_product_id == 5 || $record->item->erp_product_id == 7){
 
                        $record->update(['origin'=> 17]);
 
                    }elseif($record->item->erp_product_id == 14){
 
                        $record->update(['origin'=> 6]);
 
                    }elseif($record->item->erp_product_id == 6){
 
                        $record->update(['origin'=> 22]);
 
                    }elseif($record->item->erp_product_id == 16){
 
                        $record->update(['origin'=> 17]);
 
                    }elseif($record->item->erp_product_id == 11 
                    || $record->item->erp_product_id == 1
                    || $record->item->erp_product_id == 12
                    || $record->item->erp_product_id == 8
                    || $record->item->erp_product_id == 9){
 
                        $record->update(['origin'=> 2]);
    ...
    {      
        TblIfvalue::where('specie',null)->chunk(200,function($records){
            foreach($records as $record){
                if($record->item ?? false){

                    if($record->item->erp_product_id == 2){
 
                        $record->update(['specie'=> 4]);
 
                    }elseif($record->item->erp_product_id == 3 || $record->item->erp_product_id == 13){
 
                        $record->update(['specie'=> 6]);
 
                    }elseif($record->item->erp_product_id == 4 ){
 
                        $record->update(['specie'=> 22]);
 
                    }elseif($record->item->erp_product_id == 6 || $record->item->erp_product_id == 14){
 
                        $record->update(['specie'=> 3]);
 
                    }elseif($record->item->erp_product_id == 5){
 
                        $record->update(['specie'=> 5]);
 
                    }elseif($record->item->erp_product_id == 16){
 
                        $record->update(['specie'=> 8]);
 
                    }elseif($record->item->erp_product_id == 7 
                    || $record->item->erp_product_id == 11
                    || $record->item->erp_product_id == 1
                    || $record->item->erp_product_id == 12
                    || $record->item->erp_product_id == 8
                    || $record->item->erp_product_id == 9
    ...
    {    
        TblIfvalue::select('id','item_id','ratti_id','weight')->where('ratti_id',null)->has('item')->chunk(200, function ($ifValues) {
    ...
    {    
        $data = TblIfvalue::select('id','item_id','rate_profile_id','grade')->with('item:id,item_id,erp_product_id')->where('rate_profile_id',null) 
        ->chunk(5000, function ($ifValues) {
            foreach ($ifValues as $ifValue){
                if($ifValue->item ?? false){
                    
                    if($rateProfileId = $this->getRateProfile($ifValue)){
                        $ifValue->update(['rate_profile_id' => $rateProfileId]);
                    }
                 }
            }
        });
    ...
    public function getRateProfile($ifValue)
    {   
        return ProductGradeRateProfile::where([
            'product_id' => $ifValue->item->erp_product_id,
            'grade_id'=> $ifValue->grade,
            'status'=> 1
    ...
    {   
        ...
        // $items = TblItem::with('ifValue')->where('item_id','>',$oldItemId)->pluck('item_id')->toArray();
        // $items = TblIfvalue::whereIn('item_id',$items)->count();
        // $items = TblItem::doesNtHave('ifValue')->whereDate('created_at', '=', Carbon::today()->toDateString())->count(); 
        // dd($items);
        if($items->count() > 0){
            try{
                DB::beginTransaction(); 
    ...
    public function exportCsvMagento(){
           
       
        return Excel::download(new MagentoPriceExport, 'users-collection.xlsx');
    }
}
