  ...
  <div class="card">
    ...
    <div class="card-block table-border-style">
      <div class="table-responsiv">
        <form id="modules" action="javascript:void(0)">
          @csrf
          ...
          <ul class="wtree">
            <li>
              <span>
                  <input class="module-checkbox" type="checkbox" name="modules[{{$module->id}}]" {{ in_array($module->id,$roleModulesIds) ? "checked" : "" }} value="{{$module->id}}" data-level='module'> 
                  {{ $module->title }}
...
function updateModules(){
    ...
    var url = "{{ route('org.role.module.update') }}";
    var formData = $("#modules").serialize();
    // alert(formData);
    $.ajax({
      url: url,
      method:"POST",
      ...
      success:function(data){
        if(data.success){
          notify('Successfully Updated Modules', 'success'); 
          $("#editModules").html("");
        }
      }
    });
