...
<form id="createForm" onsubmit="event.preventDefault();">
    @csrf
        <input type="hidden" name="accountId" value="{{$subStoreManager->id }}">
       <div class="row">
        <div class="col-xl-4 col-md-12 col-12 mb-1">
        <div class="form-group">
            <label for="basicInput">Email</label> 
            @if ($subStoreManager->email_verify)
            ...
            @endif
            <input name="name" type="email" class="form-control" value="{{$subStoreManager->email }}" readonly/>
        </div>
        </div> 
        ...
        <div class="form-group">
            ...
            @if($subStoreManager->phone_verify)
            <i class="fa fa-check text-success"></i>
            <label class="label label-success">Verified</label>
            @else
            <i class="fa fa-times text-danger"></i>
            <label class="label label-danger">Not Verified</label>
            @endif
            <input  id="phone" type="text" class="form-control" name="phone" value="{{ $subStoreManager->getPhoneWithCode($subStoreManager->id)}}" readonly />
        </div>
        </div>
      
    
        @if (!$subStoreManager->email_verify || !$subStoreManager->phone_verify)
          ...
          <div class="col-xl-2 col-md-6 col-12 my-auto">
                <div class="form-group "> 
                      <label for="basicInput"></label>
        <button class="btn btn-sm btn-success m-t-15" onclick='refreshComponent("{{$subStoreManager->id}}")'>Refresh</button>
                </div>
             </div>
    </div>
        @else
 ...
 <script>
     function sendOtp(){
        ...
        sendEmail('{{ $subStoreManager->id }}');         
        getSmsToken('{{ $subStoreManager->id }}');
    }
