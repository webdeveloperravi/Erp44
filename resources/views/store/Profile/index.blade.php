...
<div class="row">
   ...
   <img src="https://colorlib.com/polygon/adminty/files/assets/images/social/img1.jpg" class="img-fluid width-100" alt="" />
   ...
   <img class="img-fluid width-100" src="https://colorlib.com/polygon/adminty/files/assets/images/social/profile.jpg" alt="">
...
<tr>
<th class="social-label b-none">Account Type
</th>
@if ($authUser->type == 'org')
<td class="social-user-name b-none text-muted">Store</td>
@elseif($authUser->type == 'lab')
<td class="social-user-name b-none text-muted">Super Company</td>
@elseif($authUser->type == 'user')
<td class="social-user-name b-none text-muted">Manager</td>
@endif
</tr>
@if ($authUser->type == 'org' || $authUser->type == 'lab')
<tr>
<th class="social-label b-none">Company Name</th>
<td class="social-user-name b-none text-muted">{{ $authUser->company_name ?? "" }}</td>
</tr>
@endif
@if ($authUser->type == 'user')
<tr>
<th class="social-label b-none">Parent Store Name</th>
<td class="social-user-name b-none text-muted">{{ $authUser->parentStore->company_name ?? "" }}</td>
</tr>
@endif

@if ($authUser->type == 'org')
<tr>
<th class="social-label b-none">Store Role</th>
<td class="social-user-name b-none text-muted">{{ $authUser->role->name ?? "" }}</td>
</tr>    
@endif
@if ($authUser->type == 'user')
<tr>
<th class="social-label b-none">Manager Role</th>
<td class="social-user-name b-none text-muted">{{ $authUser->managerRole->name ?? "" }}</td>
</tr>    
@endif 
 </table>
 <table class="table table-responsive m-b-0">
...
<script>
   ...
   function all(){
    ...
    var url = "{{ route('storeProfile.getAddresses',['/']) }}/"+accountId; 
   ...
   function createAddress(){
      var accountId ='{{ $accountId }}';
   var  url ="{{route('storeProfile.createAddress',['/'])}}/"+accountId;
   $.get(url,function(data){
...
function saveAddress(){
   ...
   $.ajax({
       url : "{{ route('storeProfile.storeAddress') }}",
       method : "POST",
...
var url = "{{route('storeProfile.editAddress',['/'])}}/"+id;
$.get(url,function(data){
   $("#createAddress").html(data)
...
function updateAddress(){
   ...
   $.ajax({
      url : "{{ route('storeProfile.updateAddress') }}",
      method : "POST",
...
function editBasicInformation(){
   var url = "{{ route('storeProfile.editBasicInformation') }}";
   $.get(url,function(data){
...
function updateBasicInformation(){
   var url = "{{ route('storeProfile.updateBasicInformation') }}";
