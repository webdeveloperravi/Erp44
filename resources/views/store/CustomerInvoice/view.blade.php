...
<div class="row">
<div class="col">
  <a href="{{route('saleInvoice')}}" class="btn btn-inverse btn-sm  m-2">Back</a>
</div>
</div>  
<div class="card">
    <!--Header ---->
    <div class="card-footer p-0" style="background-color: #04a9f5">
      <h5 class="text-white m-b-0 text-left" style="padding-top:11px; padding-bottom:11px; border-left:8px solid white; margin-left:10px; padding-left:10px;">Sale Invoice Details #{{$saleInvoice->voucher_number ?? ''}}</h5>
     </div>
    ...
    <div class="row">
      ...
      <div class="col-md-9">
        @if ($saleInvoice->userReceipt->type == 'org' || $saleInvoice->userReceipt->type == 'lab')
        <h6>To : <span>{{$saleInvoice->userReceipt->company_name ?? ''}}</span></h6>
        @endif
        @if ($saleInvoice->userReceipt->type == 'user')
        <h6>To : <span>{{$saleInvoice->userReceipt->name ?? ''}}</span></h6>
        @endif
      <h6>Date : <span>  {{Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $saleInvoice->created_at)->isoFormat('DD-MM-YYYY') }}</span>
      </h6>
      </div>
  ...
  @if (!empty($saleInvoice->ledgerDetails))
...
<tbody>
@foreach($saleInvoice->ledgerDetails as $product)
<tr>
...
<td></td>
<td>Total Amount</td>
<td>{{ number_format($saleInvoice->getSaleChallanTotalAmount($saleInvoice->id),2) }}</td>
</tr>
</tbody>
<tfoot>
  <tr>
...
<div class="card">  
  ...
  <div class="row">
     @if($saleInvoice->mediaImages->count() == 0)
      <div class="col-lg-12 col-sm-12">
      <div class="jumbotron text-center">
        <h2 class="text-center "><i class="fa fa-inbox "></i>&nbsp; Empty</h2>
         <a class="btn btn-dark mt-3" href="{{ route('ledgerMedia.index',$saleInvoice->id) }}">Click to Upload Images</a>
      </div>
     </div>
     @else
      @foreach ($saleInvoice->mediaImages as $image)
      ...
      <div class="col-md-12">
        <div class="jumbotron text-center"> 
           <a class="btn btn-dark mt-3" href="{{ route('ledgerMedia.index',$saleInvoice->id) }}">Click to Upload More Images</a>
        </div>
      </div>
      @endif
</div> 
