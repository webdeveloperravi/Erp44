...
<div class="row">
<div class="col">
  <a href="{{route('storeAccount.index')}}" class="btn btn-inverse btn-sm  m-2">Back</a>
</div>
</div>  
...
var  url ="{{route('storeAccount.edit',['/'])}}/"+accountId;
    $.get(url,function(data){
...
function subStoreIndex(){

 var url ="{{route('subStoreAccount.index',['/'])}}/"+"{{$store->id}}";
 $.get(url,function(data){
