    ...
    <div class="information" style="">
        <table width="100%">
           <tr>
              ...
              <td align="center" style="width: 100%; font-size:small; font-weight:500">{{ $ledger->userIssue->parentStore->company_name ?? "" }}</td>
              @elseif(in_array($ledger->userIssue->type,$storeTypesAll))
              <td align="center" style="width: 100%; font-size:small; font-weight:500">{{ $ledger->userIssue->company_name ?? "" }}</td>
              @endif
           </tr>
           @php
             if(in_array($ledger->userIssue->type,$storeUserTypesAll))
             {
               $address = $ledger->userIssue->parentStore->primaryAddress;

             }elseif(in_array($ledger->userIssue->type,$storeTypesAll))
             {
                $address = $ledger->userIssue->primaryAddress;
             }
           @endphp
           <tr>
     ...
     <div class="information" style="padding:0px;">
        <table width="100%"  style="padding:0%">
           <tr>
              ...
              <td align="left" style="width: 50%; font-size:small">{{ $ledger->userReceipt->company_name ?? "" }}</td>
              @endif
              @if ($ledger->userReceipt->type == 'user')
              <td align="left" style="width: 50%; font-size:small">{{ $ledger->userReceipt->parentStore->company_name ?? "" }}</td>
              @endif
              
              {{-- Ordrer Number --}}
              @if ($ledger->isPreparedOrder())
              <td align="left" style="width: 50%; font-size:small">Order No. : {{ $ledger->order->po_number ?? "" }}</td>
              @endif

           </tr>
           <tr>
              <td align="left" style="width: 50%; font-size:small">{{ $ledger->userReceipt->name ?? "" }}</td>
              @if ($ledger->isPreparedOrder()) 
              <td align="left" style="width: 50%; font-size:small">Placed On : {{ dateFormat($ledger->order->created_at) ?? "" }}</td>
              @endif  
           </tr>
           <tr>
              <td align="left" style="width: 50%; font-size:small">
              ...
              @if ($ledger->isPreparedOrder())
              <td align="left" style="width: 50%; font-size:small">
               {{ $ledger->order->buyerStore->primaryAddress->address ?? "" }},
               {{ $ledger->order->buyerStore->primaryAddress->town->name ?? ""  }}
               {{ $ledger->order->buyerStore->primaryAddress->city->name ?? "" }}-
               {{ $ledger->order->buyerStore->primaryAddress->pincode ?? ""  }},
               {{ $ledger->order->buyerStore->primaryAddress->state->name ?? "" }},
               {{ $ledger->order->buyerStore->primaryAddress->country->name ?? "" }}, 
            </td>
              @endif
           ...
           <tr> 
              ...
              @if ($ledger->isPreparedOrder())
              <td align="left" style="width: 50%; font-size:small">Approved By : {{ $ledger->order->approvedBy->name ?? "" }}</td> 
              @endif 
           </tr>
           <tr> 
              <td align="left" style="width: 50%; font-size:small">
              ...
              @if ($ledger->isPreparedOrder()) 
              <td align="left" style="width: 50%; font-size:small">Prepared By : {{ $ledger->createdBy->name ?? "" }}</td>
              @endif  
           </tr>
        </table>
     </div> 
     ...
     <div class="information">
        <table width="100%"  style="padding:0%" class="custom-table">
           <tr>
              ...
              <td align="left" style="width:9%; font-size:small">SKU</td>
              <td align="left" style="width:24%; font-size:small">Item Name</td>
              <td align="left" style="width:13%; font-size:small">HSN Code</td>
              <td align="right" style="width:9%; font-size:small">Qty.</td>
              <td align="right" style="width:9%; font-size:small">Unit</td>
              <td align="right" style="width:9%; font-size:small">Rate</td>
              <td align="right" style="width:9%; font-size:small">GST</td>
     ...
     <div class="information">
        <table width="100%"  style="padding:0%" class="custom-table">
           ...
           <tr>
              ...
              <td align="right" style="width:9%; font-size:small">{{ $detail->tax_rate }}%</td>
     ...
     <div class="information">
        <table width="100%"  style="padding:0%">
           <tr>
              ...
              <td align="right" style="width: 25%; font-size:small">{{ number_format($ledger->products_amount,2) }}</td>
           </tr>
        </table>
     </div>
     <div class="information">
     ...
     <div class="information">
        <table width="100%"  style="padding:0%">
           <tr>
              <td align="left" style="width: 50%; font-size:small">&nbsp;</td>
              <td align="right" style="width: 50%; font-size:small;">For {{ $ledger->storeReceipt->company_name ?? "" }}</td>
           </tr>
        </table>
     </div>
     <div class="information">
     ...
     <div class="information" style="padding:0px;">
        <table width="100%"  style="padding:0%">
           <tr>
              <td align="left" style="width: 33%; font-size:small">
                 _________________________________ <br>
                 {{ $ledger->userReceipt->name ?? "" }} <br>
                 Received By  
              </td>
              <td align="left" style="width: 33%; font-size:small">_________________________________<br><br>Checked By</td>
              <td align="left" style="width: 33%; font-size:small">
                 _________________________________ <br>
