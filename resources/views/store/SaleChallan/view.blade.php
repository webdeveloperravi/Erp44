...
<div class="card">
    ...
    <div class="card-footer p-0" style="background-color: #04a9f5">
      <h5 class="text-white m-b-0 text-left" style="padding-top:11px; padding-bottom:11px; border-left:8px solid white; margin-left:10px; padding-left:10px;">Sale Challan Details #{{$saleChallan->voucher_number ?? ''}}</h5>
     </div>
...
<td></td>
<td>Total Amount</td>
<td>{{ amountFormat($saleChallan->total_amount) ?? "" }}</td> 
</tr>
</tbody>
<tfoot>
  <tr>
...
<div class="card">  
  ...
  <div class="row">
     @if($saleChallan->mediaImages->count() == 0)
      <div class="col-lg-12 col-sm-12">
      <div class="jumbotron text-center">
        <h2 class="text-center "><i class="fa fa-inbox "></i>&nbsp; Empty</h2>
         <a class="btn btn-dark mt-3" href="{{ route('ledgerMedia.index',$saleChallan->id) }}">Click to Upload Images</a>
      </div>
     </div>
     @else
      @foreach ($saleChallan->mediaImages as $image)
      ...
      <div class="col-md-12">
        <div class="jumbotron text-center"> 
           <a class="btn btn-dark mt-3" href="{{ route('ledgerMedia.index',$saleChallan->id) }}">Click to Upload More Images</a>
        </div>
      </div>
      @endif
</div> 
  ...
  var messageTop2 =`Report from UID : ` + authUserId + "     Name : " + authUserName + "     Role : " + userRole + "     Company : " + companyName;
    var table = $('#table_id2').DataTable({
      ...
      action: function ( e, dt, button, config ) {
         window.open("{{ route('saleChallan.detailsPrint',['/']) }}/"+"{{ $saleChallan->id }}", '_blank'); 
      }
      } 
      ],
