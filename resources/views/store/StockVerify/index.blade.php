...
<div id="app">
  <stock-verify 
   ...
   route-get-accounts="{{route('stockVerify.getAccounts')}}"
   v-bind:products="{{ $products }}" 
  v-bind:grades="{{ $grades }}" v-bind:rattis="{{ $rattis }}"
     route-save-gins="{{ route('stockVerify.saveGins') }}"
     route-get-products="{{ route('stockVerify.getProducts') }}"
  ></stock-verify>  
</div>
 ...
 function notify(message, type){
     ...
     function getProducts(){
      $.ajax({
         url : "{{ route('stockVerify.getProducts') }}",
         method : "POST",
