...
<div class="container">
   ...
   <li style="margin-left:30px;" class="saab">
      ...
      @if ($group->user_store_id == '0' || $group->user_store_id == auth('store')->user()->id)
   ...
   <li style="margin-left:30px;" class="saab">
    <div class="listree-submenu-heading btn btn-block {{ $group->status == 0 ? "btn-danger" : "btn-inverse"}}  text-left mt-1" >{{ $group->name }}
      @if ($group->user_store_id == auth('store')->user()->id)
        ...
        <button class="btn btn-sm btn-warning p-1 mr-1 float-right" onclick="edit({{$group->id}})" style="width:60px;"> edit</button>
        @endif
        </div>    
