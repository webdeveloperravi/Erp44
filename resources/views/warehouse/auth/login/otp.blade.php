   ...
   <div class="row">
      <div class="col-sm-12">
         ...
         <form  id="otpForm" onsubmit="event.preventDefault()">
            @csrf
            <input type="hidden" name="id" value="{{$store->id}}" id="id">
...
<script type="text/javascript">
   ...
   function sendSms(){
      ...
      //For Whats App
      var number = "{{$store->getWhatsAppWithCode($store->id)}}";
      var message = "9 Gem Store Login Code "+"{{$store->otp_code}}";
   ...
   //              "content": "9 Gem Store Login Code "+"{{$store->otp_code}}",
   //          }
   //      ]
   //        }
   //      );
   ...
   function resendOtp(){
    let id=$("#id").val();
    let url = "{{ route('warehouse.resendOtp') }}";
